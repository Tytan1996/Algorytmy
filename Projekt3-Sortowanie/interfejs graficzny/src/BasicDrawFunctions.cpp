...
void onStart()
{
    ...
    newCollection.addNew({convertSize(0.0f,550.0f),convertSize(200.0f,50.0f),"Open",false,3,9,[](){ApplyPresetStruct(openPreset(openFile()));}});
...
void display()
{
    ...
    }else
    {
        ...
        }else
        {
            ...
            if(!benchState)newCollection.addNew({convertSize(0.0f,140.0f),convertSize(200.0f,50.0f),"Save Preset",false,0,2,[](){savePreset(saveFile(),GeneratePresetStruct("File generated by program"));}});
        }
    }
