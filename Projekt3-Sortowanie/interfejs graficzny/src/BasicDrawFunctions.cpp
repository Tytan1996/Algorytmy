...
void AiSD::display()
{
    ...
    }else
    {
        ...
        if(!getProcessing())
        {
            ...
            if(!benchState&&openMenuActive==false)
            {
                newCollection.addNew({convertSize(0.0f,140.0f),convertSize(200.0f,50.0f),"Save Sorted",false,0,2,[](){savePreset(saveFile(),GeneratePresetStruct("File generated by program",false));}});
                newCollection.addNew({convertSize(0.0f,210.0f),convertSize(200.0f,50.0f),"Save Unsorted",false,0,2,[](){savePreset(saveFile(),GeneratePresetStruct("File generated by program",true));}});
            }
        }
        if(openMenuActive==true)
        {
            if(alreadySorted==false)
                newCollection.addNew({convertSize(600.0f,0.0f),convertSize(200.0f,50.0f),"Sort",false,0,0,[](){alreadySorted=true;Start(true);}});
            else
                newCollection.addNew({convertSize(0.0f,140.0f),convertSize(200.0f,50.0f),"Save",false,0,2,[](){savePreset(saveFile(),GeneratePresetStruct("File generated by program",false));}});
