...
void AiSD::display()
{
    ...
    }else
    {
        ...
        if(!getProcessing())
        {
            ...
            {
                newCollection.addNew({convertSize(0.0f,140.0f),convertSize(200.0f,50.0f),"Save Sorted",false,7,14,[](){savePreset(saveFile(),GeneratePresetStruct("Sorted table",false));}});
